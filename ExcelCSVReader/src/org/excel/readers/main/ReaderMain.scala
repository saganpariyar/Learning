...
//adding thrid file 
// Create an dataframe for fwd_risk_1
val fwdRisk_2File = sc.textFile("C:/sagan/fractal/My Workspaces/TrainingWKS/ExcelCSVReader/FRW_risk2.csv")
// The schema is encoded in a string
val fwdRisk_2SchemaString = "scenario:String prod_dsc:String cusip:String valuation_date:String portfolio_date:String All_200:Double "+
"base:Double basebalance:Double pv01:Double pv200:Double goal:Double"
// Generate the schema based on the string of schema
val fwdRisk_2Schema =
  StructType(
    fwdRisk_2SchemaString.split(" ").map( fieldName => if(fieldName.split(":")(1)=="String")StructField(fieldName.split(":")(0), StringType, true) 
        else if(fieldName.split(":")(1)=="Double") StructField(fieldName.split(":")(0), DoubleType, true) else StructField(fieldName.split(":")(0), LongType, true)))
// Convert records of the RDD (people) to Rows.
val fwdRisk_2RowRDD = fwdRisk_2File.map(_.split(",")).map(p => Row(p(0),p(1),p(2),monthEndDate(p(3)),monthEndDate(p(4)),p(5).toDouble,p(6).toDouble,
    p(7).toDouble,p(8).toDouble,p(9).toDouble,p(10).toDouble ))
// Apply the schema to the RDD.
var fwdRisk_2DataFrame = sqlContext.createDataFrame(fwdRisk_2RowRDD, fwdRisk_2Schema)
// Register the DataFrames as a table.
fwdRisk_2DataFrame.registerTempTable("fwdRisk_2")
  ...
  val combineRiskDF = sqlContext.sql("select aa.*, bb.CPR, bb.CDR,  bb.survival, bb.balance, cc.load from  fwdRisk_1 aa join fwd_arm_prepay bb "+
    "on aa.scenario = bb.scenario and aa.prod_dsc = bb.prod_dsc and aa.cusip = bb.cusip and aa.valuation_date = bb.valuation_date and "+
    "aa.portfolio_date = bb.portfolio_date join fwdRisk_2 cc on  aa.scenario = cc.scenario and aa.prod_dsc = cc.prod_dsc and "+
    "aa.cusip = cc.cusip and aa.valuation_date = cc.valuation_date and aa.portfolio_date = cc.portfolio_date ")
