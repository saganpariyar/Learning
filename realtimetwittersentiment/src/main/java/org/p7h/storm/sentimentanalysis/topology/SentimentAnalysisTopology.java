...
import org.p7h.storm.sentimentanalysis.spouts.FileSpout;
...
public final class SentimentAnalysisTopology {
	...
	public static final void main(final String[] args) throws Exception {
		...
		jmsBolt.setJmsMessageProducer((session, input) -> {
            final String json = "{\"tweet\":\"" + input.getString(0) + "\", \"sentiment\":" + input.getInteger(1) + "}";
		...
		try {
			...
			topologyBuilder.setSpout("filespout", new FileSpout());
			topologyBuilder.setBolt("sentimentcalculatorbolt", new SentimentCalculatorBolt())
					.shuffleGrouping("filespout");
			
			topologyBuilder.setBolt("jmsBolt", jmsBolt)	.shuffleGrouping("sentimentcalculatorbolt");
