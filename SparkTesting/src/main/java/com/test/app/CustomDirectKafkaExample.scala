...
object CustomDirectKafkaExample {
  ...
  val CHECKPOINT_DIRECTORY_REQUEST = "/home/saganpariyar"
  ...
  def main(args: Array[String]) {
    ...
    sparkConf.setAppName("CustomDirectKafkaExample")
    ...
    val topicsSet = "testtopic".split(",").toSet
    val ssc =  setupSsc(topicsSet, kafkaParams, checkpointDir,sc)
    /* Start the spark streaming   */
    ssc.start()
    ssc.awaitTermination();
  }//main() ends

  def setupSsc(topicsSet: Set[String], kafkaParams: Map[String, String], checkpointDir: String, sc:SparkContext)(): StreamingContext = {
  ...
  private def saveOffsets(zkClient: ZkClient,zkHosts:String, zkPath: String, rdd: RDD[_]): Unit = {
    ...
    offsetsRanges.foreach(offsetRange => print(s"Using ${offsetRange}"))
